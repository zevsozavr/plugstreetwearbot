  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', paddingBottom: 128 }}>
      ...
      <main style={{ paddingTop: 64 }}>
        <section style={{ position: 'relative', width: '100%', height: 618, display: 'flex', alignItems: 'flex-end', padding: '0 24px 48px', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 10, maxWidth: 576 }}>
            ...
            <button className="active:scale-95 glow-hover" style={{
        ...
        <section style={{ padding: '32px 24px', overflowX: 'auto', position: 'sticky', top: 64, zIndex: 40, background: 'rgba(10,14,26,0.8)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(255,255,255,0.05)' }} className="hide-scrollbar">
              ...
              <div key={p.id} onClick={() => navigate(`/product/${p.id}`)} style={{ minWidth: 280, cursor: 'pointer' }} className="group">
                <div style={{ position: 'relative', height: 380, borderRadius: 16, overflow: 'hidden', marginBottom: 16 }} className="glass-card">
                  ...
                  <div style={{
                    ...
                    <button style={{
                      width: 48, height: 48, borderRadius: '50%', background: 'var(--primary)', color: 'var(--on-primary)',
                      display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.3)',
                ...
                <p style={{ color: 'var(--primary)', fontSize: 14, fontWeight: 600, marginTop: 2 }}>{p.price.toLocaleString()}₴</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section style={{ padding: '0 24px 48px' }}>
          <div className="glass-card group" style={{ position: 'relative', width: '100%', height: 320, borderRadius: 24, overflow: 'hidden', borderColor: 'rgba(125,211,252,0.2)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, alignItems: 'start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              {products.filter((_, i) => i % 2 === 0).slice(0, 2).map((p) => (
                <div key={p.id} style={{ cursor: 'pointer' }} className="group">
                  <div onClick={() => navigate(`/product/${p.id}`)} style={{ position: 'relative', aspectRatio: '3/4', borderRadius: 16, overflow: 'hidden', marginBottom: 12 }} className="glass-card">
                    <img src={p.image} alt={p.name} loading="lazy"
                      style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.5s' }}
                      className="group-hover:scale-110" />
                    <div style={{
                      position: 'absolute', inset: 0, background: 'rgba(125,211,252,0.1)', opacity: 0,
                      transition: 'opacity 0.3s', display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }} className="group-hover:opacity-100">
                      <button style={{ padding: '10px 20px', background: 'var(--primary)', color: 'var(--on-primary)', borderRadius: 9999, fontSize: 10, fontWeight: 700, boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>ADD TO BAG</button>
                    </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24, paddingTop: 48 }}>
              {products.filter((_, i) => i % 2 === 1).slice(0, 2).map((p) => (
                <div key={p.id} style={{ cursor: 'pointer' }} className="group">
                  <div onClick={() => navigate(`/product/${p.id}`)} style={{ position: 'relative', aspectRatio: '4/5', borderRadius: 16, overflow: 'hidden', marginBottom: 12 }} className="glass-card">
                    <img src={p.image} alt={p.name} loading="lazy"
                      style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.5s' }}
                      className="group-hover:scale-110" />
                    <div style={{
