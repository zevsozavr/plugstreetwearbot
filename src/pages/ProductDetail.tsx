      ...
      <main style={{ flex: 1, overflow: 'auto', position: 'relative', zIndex: 10, paddingBottom: 32 }}>
        <section style={{
          ...
          <div style={{ position: 'relative', width: '100%', maxWidth: 320, aspectRatio: '3/4' }}>
            <div style={{
              position: 'absolute', inset: 0, background: 'rgba(243,235,227,0.08)',
        ...
        <section style={{ padding: '0 var(--pad)', marginTop: -32 }}>
          <Glass card style={{ borderRadius: 'var(--rounded-2xl)', padding: 32, display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <span style={{ font: 'var(--font-label)', fontSize: 11, color: 'var(--on-surface-variant)', marginLeft: 8, padding: '2px 8px', borderRadius: 'var(--radius-full)', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>{t('product.condition.' + product.condition.toLowerCase().replace(/\s+/g, '_'))}</span>
                ...
                {product.colors.map((c) => (
                  <button key={c.name} onClick={() => setColor(c.name)}
                    style={{
                      ...
                      boxShadow: color === c.name ? '0 0 12px rgba(243,235,227,0.25)' : 'none',
                      cursor: 'pointer',
                    }} />
                ))}
              </div>
                ...
                {product.sizes.map((s) => (
                  <button key={s} onClick={() => setSize(s)}
                    style={{
                      ...
                      background: size === s ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.04)',
                      ...
                      boxShadow: size === s ? '0 0 10px rgba(243,235,227,0.15)' : 'none',
                      cursor: 'pointer',
                    }}>
                    {s}
                  </button>
